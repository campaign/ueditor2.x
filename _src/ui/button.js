//button 类
UE.ui.define('button', {
    tpl: '<<%if(!texttype){%>button class="btn<%if (name) {%> edui-button-<%= name %><%}%>" <%}else{%>a class="edui-text-btn"<%}%><% if(title) {%> data-original-title="<%=title%>" <%};%>> ' +
        '<% if(icon) {%><i class="icon-<%=icon%> edui-icon"></i><% }; %><%if(text) {%><span class="edui-button-label"><%=text%></span><%}%>' +
